...
const PlaceScreen = () => {
  ...
  console.log(route.params.seletedDate.startDate)
  const data = [
    {
      id: "0",
      place: "Bangalore",
       ...
       switch(filter){
         case "cost:High to low":
          ...
          case "cost:Low to high":
           searchPlaces.map((val)=>val.properties.sort(comparation));
           setSortedData(searchPlaces);
           break;
       }
  } 
    ...
    <View>
      ...
      >
        ...
        <Pressable onPress={()=> navigation.navigate("Map",{
          ...
          <FontAwesome5 name="map-marked-alt" size={22} color="gray" />
          <Text>Map</Text>
        </Pressable>
      </Pressable>
          ...
          .map((item) =>
            item.properties.map((property, index) => (
              <PropertyCard
                ...
                seletedDates={route.params.seletedDate}
                property={property}
      ...
      <ModalContent style={{width:"100%",height:500}}>
            <View style={{flexDirection:'row'}}>
               <View style={{borderRightWidth:1,borderColor:"#E0E0E0",marginVertical:10,flex:2,height:100,marginRight:20}}>
                  <Text style={{textAlign:'center'}}>Sort</Text>
               </View>
               <View style={{flex:3,margin:1}}>
                 {filters.map((item,index)=>(
                   <Pressable onPress={()=>setSelectedFilter(item.filter)} style={{flexDirection:'row',marginVertical:10,alignItems:'center'}} key={index}>
                      {selectedFilter.includes(item.filter) ? (
                        <FontAwesome name="circle" size={18} color="grren"/>
                      ) : (
                        <Entypo name="circle" size={18} color="black" />
                      )}
                   <Text style={{fontWeight:'500',marginLeft:6,fontSize:16}}>
                   {item.filter}
                   </Text>
                   </Pressable>
                 ))}
